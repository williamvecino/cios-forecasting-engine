...
- **Simulate Adoption Reaction:** Downstream of Respond (accessible via button). Tests how defined segments (Early Adopters, Persuadables, Late Movers, Resistant) respond to specific materials (uploaded files or pasted text). Uses existing gates, barriers, triggers — never creates new constraints. Outputs: adoption likelihood (%), confidence, primary reaction, barrier sensitivity, trigger condition, material effectiveness. Adoption likelihood is capped at constrained_probability + 15pp to prevent impossible predictions. Uses `/api/ai-simulate/reaction` endpoint with multipart file upload support.
- **Assumption Registry:** Silent background system that automatically extracts and tracks all inferred/explicit assumptions underlying forecasts, constraints, and recommendations. Each assumption is structured with category (Clinical, Regulatory, Market Access, Competitive, Behavioral, Operational), status (active/challenged/invalidated), confidence level, version tracking, and gate linkage. Auto-triggers after Decide and Respond steps complete. Accessible via "View Assumptions" button in sidebar on steps 3-6. Uses `/api/ai-assumptions/extract` endpoint. Cached per case in `cios.assumptions:${caseId}`. Existing assumptions are passed back on re-extraction so AI can update status when evidence changes.

...
- **Strategic Relevance Page Hierarchy (Respond page):** 4-section layout with visibility rules. (1) Decision Snapshot (always visible): Decision, probability, most likely outcome, primary blocker, fastest way to improve. (2) Interpretation (always visible): environment strength, success definition, why the probability looks like this, what would change the forecast. (3) Needle Movement (visible): up/down drivers, strategic/tactical actions. (4) Diagnostics (hidden by default): signal details, fragility, confidence ceiling, compression, sensitivity, consistency. Diagnostics auto-reveal when: confidence is Low, probability shift ≥10pp, or signals conflict. User can manually toggle via "Explain" button. Forecast data fetched independently for diagnostics even when respond output comes from cache.

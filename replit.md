...
- **Adoption Distribution Forecast Model (v3 — ceiling-based):** Final probability is computed by (1) building a Beta adoption distribution from the Bayesian posterior, confidence level, signal count, and evidence diversity; (2) computing an achievable ceiling from gate constraints (severity × binding fraction, independent of signal concentration); (3) scaling the distribution mean to fit within the ceiling while preserving concentration (signal spread); (4) calculating P(adoption ≥ threshold) from the adjusted distribution's CDF. Readiness is a separate intermediate variable (product of 1−severity across gates). Gate Domination diagnostic flags when gates account for >50% of probability reduction and >5pp impact. Implementation: `adoption-distribution.ts` (both backend and frontend). Gate severity: strong=0, moderate=0.25, weak=0.55, unresolved=0.85.

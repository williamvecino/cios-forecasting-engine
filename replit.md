...
- **MIOS Evidence Verification Guardrail:** Automated evidence verification layer (`lib/evidence-verification.ts`) that extracts identifiers (PMID, DOI, NCT) from signal evidence text and verifies them against PubMed, CrossRef, and ClinicalTrials.gov registries. Red-flag detection catches narrative citations without verifiable identifiers, perfectly rounded percentages, and nonstandard journal names. Verification runs automatically on signal transitions to validated/active status. Endpoints: `POST /api/signals/:id/verify` (single), `POST /api/cases/:caseId/signals/verify-all` (batch), `POST /api/signals/check-evidence` (ad-hoc). Signal schema includes `identifierSource`, `identifierType`, `identifierValue`, `verificationStatus` (verified/invalid/flagged/unverified), `registryMatch`, `verificationTimestamp`, `verificationRedFlags`. UI shows color-coded verification badges on signal cards.
- **Archetype Signal Grammars (9 Archetypes):** Each CaseFrame definition includes `archetypeLabel`, `archetypeCategory` (core_clinical_commercial | enterprise_system), `judgmentQuestions` (3 key questions), `correctSignalTypes` (signals to generate), and `incorrectSignalTypes` (signals to reject). 5 core archetypes + 4 enterprise archetypes (investment_portfolio, operational_execution, strategic_partnership, policy_environment). Frame constraints injected into LLM prompts include judgment questions and signal type rules.

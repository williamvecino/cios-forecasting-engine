...
- **MIOS/BAOS Workbook Import (Input Adapter):** Pure input adapter (`artifacts/cios-frontend/src/lib/workbook/`) that reads only `CIOS_Signal_Export` sheet, imports only `ActiveFlag=Yes` rows, maps 6 columns (ProgramID, SignalLabel, Direction, Strength, Confidence, WhyItMatters) into CIOS signal structure. Replace-only mode — clears existing signals and replaces with imported set. Triggers existing forecast logic via gate recalculation. Does not read other sheets, interpret evidence, or modify judgment logic. Strict enum validation (unknown direction/strength/confidence values are skipped). UI: drag-drop dialog with signal summary and provenance drawer.

...
- **Bayesian Forecast Engine:** Implemented in `forecast-engine.ts`, this module calculates posterior probabilities using prior odds, a correlation-aware signal LR product (with soft compression cap at log(LR)=5.0), and an exponential net actor translation. Integrity metrics (`_integrityMetrics`) are embedded in every forecast output.
- **Signal Detection & Review:** An AI-powered layer extracts candidate signals from various documents, followed by a workflow for review, confirmation, or rejection. A signal completeness check ensures 14-domain coverage. Signals are validated on ingest: `signalType` must match the canonical 10-type taxonomy, `direction` must be Positive/Negative, `strengthScore`/`reliabilityScore` must be numeric 1–5. Duplicate detection (Jaccard similarity >0.6) warns on creation. Signals support an optional `correlationGroup` field for dependency-aware LR aggregation.

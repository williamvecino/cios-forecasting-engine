...
- **Judgment Integrity Test Set:** 27 locked tests in `judgment-engine.test.ts` across 8 groups: signal dominance (3), gate stress (3), confidence integrity (3), static regression (1), single-variable sensitivity (4), contradiction stress (6), audit trail completeness (3), narrative integrity (4). Tests cover: deterministic output (same input = same result), causal sensitivity (changing one variable moves output in expected direction), contradiction detection (positive majority can't produce negative polarity, high gap can't produce high confidence), and narrative coherence (reasoning/posture must match probability band). Run with `pnpm --filter @workspace/cios-frontend test`.

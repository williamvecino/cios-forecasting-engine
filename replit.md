...
- **Evidence Classification Gate:** Every signal is classified before entering the pipeline: `DynamicSignal` (has source, date, event specificity, novelty — counts toward posterior), `StructuralContext` (background context — informs priors/explanation only), `InterpretationNote` (opinion/narrative — explanation layers only), `RejectedArtifact` (blocked from forecasting). Only signals with `countTowardPosterior=true` enter LR multiplication. Classification runs on POST/PUT via `classifyEvidence()`. DB columns: `evidence_class`, `count_toward_posterior`.

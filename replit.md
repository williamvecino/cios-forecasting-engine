...
- **Structured Pivotal Evidence Search:** `POST /api/cases/:caseId/pivotal-search` generates structured search queries across 5 categories (Pivotal Trials, Label/Approval Data, Guidelines, Safety, Payer/Access) using the case's drug name and indication. Returns 10-15 evidence candidates with trial names, PMIDs, one-sentence findings, signal types, and directions. All candidates start as `countTowardPosterior: false`. Analyst approves individual candidates via `POST /api/cases/:caseId/pivotal-search/approve`, which creates signals with `countTowardPosterior: true`. Frontend component `PivotalEvidenceSearch.tsx` renders on the signals page with category-grouped results and approve/reject buttons per candidate.

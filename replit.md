...
The signals page calls `POST /api/ai-signals/generate` which detects the therapeutic area as context but evaluates each case independently on its own merits. Canonical cases (oncology, dermatology/Mallia, CNS/LAI, infectious disease) are archetypes that teach adoption dynamics, NOT templates to copy. The AI reasons about what drives adoption for the specific product type. Signals are framed as analytical drivers (not fabricated facts). Therapeutic area is stored in `localStorage.cios.therapeuticArea` for downstream use. ARCHITECTURAL RULE: Never hardcode domain-specific signal templates — the AI must evaluate brand-specific signals for each new case.

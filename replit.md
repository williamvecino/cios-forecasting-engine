...
- **Signal Dependency & Redundancy Control Layer:** Prevents posterior inflation from causally related signals. Raw signals are analyzed for lineage relationships, clustered by root evidence, and compressed before posterior calculation. Echoes contribute near-zero incremental value; translations contribute modest value. Produces concentration warnings when >3 signals share root evidence, >50% posterior from one lineage, or echo density is high. New signal fields: rootEvidenceId, signalLineage, sourceCluster (10 types), dependencyRole (Root/Direct derivative/Second-order derivative/Independent), lineageConfidence, novelInformationFlag, echoVsTranslation. Metrics: noveltyScore, echoDensity, evidenceDiversityScore, posteriorFragilityScore, concentrationPenalty. API: GET /api/cases/:caseId/signal-dependency. Frontend: SignalDependencyPanel on signals page. Canonical flow: Question → Raw Signals → Dependency/Lineage Analysis → Cluster Compression → Posterior Update.

...
- **5-Phase Evidence Pipeline (Full Document Fetch):** `POST /api/cases/:caseId/evidence-pipeline` — automated 5-phase pipeline: Phase 1 (Sponsor ID + Known Trials), Phase 2 (Source Discovery via PubMed eutils API, ClinicalTrials.gov v2 API, FDA openFDA API), Phase 3 (Full Document Fetch with API-based text extraction for PubMed abstracts via efetch and ClinicalTrials.gov study data via JSON API, plus HTML/PDF scraping fallback with cheerio), Phase 4 (LLM extraction requiring verbatim sourceQuote substrings from document text, batch 3 docs per call), Phase 5 (dedup, sort by tier). Key files: `evidence-pipeline.ts` (orchestrator), `document-fetcher.ts` (PubMed/CT.gov/FDA API discovery + HTML/PDF fetch), `authoritative-sources.ts` (shared config). Concurrency: 2 workers with 400ms PubMed rate-limit delay. Results stored in searchResultStore for approve workflow. Frontend: "Find Evidence" button in PivotalEvidenceSearch.tsx with pipeline progress display, document fetch details panel, and pipeline log viewer.
- **External Extraction Service Integration:** Optional Colab-hosted Flask+ngrok microservice for enhanced document extraction (PDFs, SEC filings, complex HTML). Configured via System > Configuration panel or `POST /api/extraction-service/configure`. Status check: `GET /api/extraction-service/status`. When configured, `document-fetcher.ts` routes PDFs/SEC URLs to external service first, uses PubMed external endpoint, and falls back to external for any local extraction yielding <200 chars. Colab notebook: `cios-document-extractor.ipynb` with `/health`, `/extract`, `/pubmed`, `/batch` endpoints. Service URL stored in `CIOS_EXTRACTION_SERVICE_URL` env var (runtime-only, set via API). Frontend: `ExtractionServiceConfig.tsx` on System page shows connection status badge (connected/unreachable/not configured) with Connect/Clear controls.

...
- **Signal Precedent Library (CIOS v18):** Precedent-based signal weighting with governance controls. 10 fixed signal types across A/B/C reliability tiers with assigned LRs (0.45–1.1475). Signal types are mapped from the 22 existing type/direction combinations to precedent categories. The `signal_precedent_library` DB table stores the canonical library. Engine version tracking (`CIOS_v18`) stamped on each case after recalculation. Run receipts stored in calibration log snapshots with `_engineVersion` and `_precedentMappings` fields. API endpoints: `GET /api/precedent-library`, `POST /api/precedent-library/seed`, `GET /api/precedent-library/lookup?signalType=X&direction=Y`, `GET /api/engine-version`. Key files: `artifacts/api-server/src/lib/precedent-lookup.ts` (mapping + lookup logic), `artifacts/api-server/src/routes/precedent-library.ts` (API routes), `lib/db/src/schema/signal-precedent-library.ts` (DB schema).

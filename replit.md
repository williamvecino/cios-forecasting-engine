...
**Driver Impact Distribution (Proportional):** The forecast page distributes the total probability shift across individual signals proportionally rather than rounding each `deltaIfRemoved` independently. Uses largest-remainder method to ensure rounded impacts sum to the total shift without loss. Upward and downward signals are allocated separately. When gates are present, the gate-constrained view shows gate-derived pressure cards (Most Sensitive Gate, Total Upward Pressure, Total Downward Pressure) computed from individual gate scenarios only (composite "all gates" scenarios excluded to avoid double-counting).
...
**Signal-to-Gate Mapping Engine (Step 2):** `signal-gate-engine.ts` provides deterministic signal→gate mapping and automatic gate recalculation. Architecture: (1) Each accepted signal is mapped to the best-matching gate via keyword scoring (category keywords ×3, family keywords ×2, text overlap ×0.5); signals with score <1 are unmapped (not force-assigned). (2) Evidence weight = strength × reliability × direction_multiplier (positive=+1, negative=-1, neutral=0). (3) Gate status shifts: net_evidence ≥8 → +1 level, ≥20 with 3+ signals → +2 levels; same thresholds for negative. Two-level shifts require minimum 3 corroborating signals. (4) `triggerGateRecalculation()` runs after every accept, dismiss, custom add, event convert, and edit commit — updates eventGates state + localStorage `cios.eventDecomposition:${caseKey}`. (5) Base gates (from AI) stored separately to enable accurate incremental recalculation. (6) Signal Impact panel: 8-second transient notification showing gate affected, status change (or "gate already strong" ceiling), and forecast before/after. (7) Signal Diagnostics panel: expandable view grouped by gate — shows per-gate signal count, evidence weights, status transitions, and overall forecast impact. All state cleared on context/question change to prevent cross-case contamination.

...
- **Executive Judgment Layer:** A post-forecast judgment engine (`judgment-engine.ts`) that classifies case type from gate/signal patterns, retrieves closest analog cases via the analog-context API, compares current gate/signal patterns to historical outcome patterns, and generates a structured Executive Judgment block. Outputs: most likely outcome verdict, probability, confidence level, reasoning, analog pattern summary, convergence note, and reversal triggers. Rendered at the top of Step 3 (above comparison circles). Fully deterministic — no AI. Gracefully handles missing analog data.

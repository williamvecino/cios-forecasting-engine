...
**CAL-03 target update — April 2026.** Target posterior updated from 0.29 → **0.60**. Reason: 0.29 was an old engine artifact set against a different signal stack and was directionally wrong for a TRUE outcome (a target <0.50 implies engine should predict FALSE, but truth is TRUE). Current 6-signal stack (5 Eligible: 3 Positive LRs of 1.35/1.20/1.08 + 2 Negative LRs of 0.82/0.90 with 1 ContextOnly Negative LR 0.88) produces a raw Bayesian posterior of **61.2%** from prior 0.55 — direction-correct for TRUE. Engine output of ~47% reflects additional `bayesianActorFactor` (0.75) and `actorAdjustmentFactor` (0.93) dampening on top of the raw signal LR product; investigating that dampening is a separate engine-behavior question.

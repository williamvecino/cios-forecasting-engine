...
- **Signal Lifecycle & Audit System:** Signals follow a lifecycle: `candidate → reviewed → validated → active → archived/rejected`. Human-created signals default to `active`; system/agent-created signals default to `candidate`. Transition endpoint: `POST /api/signals/:signalId/transition { action }`. Validation gates enforce required fields (`sourceLabel`, `observedAt`) before `validated`/`active` status. All transitions, edits, and deletions are audit-logged in `audit_logs` table (accessible via `GET /api/audit-logs`). Forecast engine, scenario adapter, and portfolio routes filter signals to `status="active"` only. Signal schema includes: `status`, `createdByType`, `createdById`, `strength`/`reliability` (text labels), `sourceLabel`, `sourceUrl`, `evidenceSnippet`, `observedAt`, `notes`, `updatedAt`. Valid transitions exported from `lib/db/src/schema/signals.ts`.
- **Signal Taxonomy:** 10 core signal types organized into 3 groups (evidence, market, execution), defined in `signal-taxonomy.ts` (frontend + API server). Types: GUIDELINE_INCLUSION, COMPETITOR_COUNTERMOVE, REGULATORY_CLINICAL, PHASE_III_CLINICAL, KOL_ENDORSEMENT, OPERATIONAL_FRICTION, ACCESS_COMMERCIAL, PAYER_ACCESS, MARKET_ADOPTION, CAPACITY_INFRASTRUCTURE. New layer types (Payer, Adoption, Capacity) each have 7 subtypes. Legacy types (Field intelligence, Competitor counteraction, Access friction, Experience infrastructure) are preserved in LR config for backward compatibility. LR ranges defined in `lr-config.ts` (both `lib/db` and `artifacts/cios-frontend`). `resolveLegacyType()` and `resolveSignalLabel()` functions map old labels to new taxonomy.
...
- **Question Definition Layer (Step 1):** A pre-case classification and structuring layer. User enters free-text; system classifies question type (binary, comparative, ranking, threshold, timing), extracts fields (subject, outcome, entities, time horizon, comparator, success metric), identifies missing required fields, and progressively asks for them. Users can manually override question type via type-selector buttons and click any detected field to edit inline. Only when all required fields are filled does the "Create Forecast Case" button activate. Utilities in `lib/question-definition/` (classifier, parser, missing-fields, interpreter, case-mapper). Duplicate signal prevention on both frontend (exact match) and API (409 for exact ID or >85% similarity). Groups field is optional for binary/timing questions (only required for comparative/ranking). Parser uses multi-strategy subject extraction: exact brand match → "for/of X" → contextual domain patterns → sentence structure ("Will [subject] [verb]"). Time horizon inferred as "12 months" when adoption-related keywords present but no explicit time reference.
- **Workflow Test Harness:** `tests/workflow-test.ts` validates the full Question→Information→Forecast→Decision pipeline for 10 predefined questions. Run via `npm run test:workflow`. Tests parser extraction, case creation, signal addition, forecast generation, sensitivity analysis, and decision recommendation. Results saved to `tests/workflow_test_results.json`, fixes to `tests/workflow_fix_log.json`, blockers to `tests/workflow_blockers.md`.
- **Forecast Readiness Gate:** All forecast tabs (Current Forecast, Scenario Planning, Driver Impact) check signal readiness before calling the forecast engine. Readiness stored in localStorage scoped by caseId (`cios.signalReadiness:${caseId}`). Requires at least 1 confirmed driver + 1 supporting signal.

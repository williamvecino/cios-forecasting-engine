...
- **Constraint Decomposition Layer:** `constraint-drivers.ts` provides a canonical dictionary of 19 abstract constraint categories (operational readiness, access readiness, payer access, behavioral readiness, clinical confidence, clinical evidence, workflow integration, market readiness, adoption readiness, infrastructure readiness, execution risk, implementation complexity, stakeholder alignment, system capacity, resource availability, training readiness, regulatory readiness, reimbursement readiness, guideline endorsement), each mapped to 5 concrete drivers with severity/probability/reach scores. Impact scoring = severity × probability × reach × status multiplier (weak=1.0, unresolved=0.9, moderate=0.6, strong=0.2). Ranks: ≥300 High, ≥150 Moderate, else Low. `enforceDecomposition()` throws if abstract constraint has no drivers. Integrated into judgment engine: `_audit.constraintDecomposition` array with per-gate drivers. DECOMP-ENFORCEMENT integrity check fires if enforcement fails. Judge page shows "Constraint Decomposition — Ranked Drivers" panel in audit trail.
- **Judgment Integrity Test Set:** 32 locked tests in `judgment-engine.test.ts` across 8 groups: signal dominance (3), gate stress (3), confidence integrity (3), static regression (1), single-variable sensitivity (4), contradiction stress (6), audit trail completeness (7), narrative integrity (5). Tests cover: deterministic output (same input = same result), causal sensitivity (changing one variable moves output in expected direction), contradiction detection (positive majority can't produce negative polarity, high gap can't produce high confidence), narrative coherence (reasoning/posture must match probability band), and constraint decomposition (audit structure, abstract gates have drivers, drivers sorted by impact, enforcement throws for unmapped abstracts, reasoning includes specific driver names). Run with `pnpm --filter @workspace/cios-frontend test`.

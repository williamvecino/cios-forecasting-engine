...
**Forecast-Derived Decision Logic:** The Decide page (Step 4) is architecturally constrained to derive all barriers and actions from Step 3's forecast gates. Frontend reads `cios.eventDecomposition:${caseId}` from localStorage and passes `forecastGates`, `brandOutlookProbability`, `constrainedProbability` to `POST /api/ai-decide/generate`. Server-side enforcement: (1) AI prompt receives gates with explicit derivation instructions, (2) barrier_diagnosis items include `source_gate_id` linking to forecast gates, (3) gate status→barrier ceiling enforcement (strong gate → barrier cannot be High), (4) weak/unresolved gates auto-generate linked actions if AI missed them, (5) integrity validator checks all barriers and actions for valid gate linkage, returns `_integrity: { gate_linked, warnings, valid }`. Frontend renders: "Gate-derived" / "Forecast-derived" labels, gate name on each barrier domain, gate name + forecast_dependency on each action, integrity warning banner when linkage is incomplete. Barrier polarity rule: readiness and barrier are inversely correlated (High readiness → Low barrier); forbidden pairs (High:High, Low:Low) auto-corrected server-side.

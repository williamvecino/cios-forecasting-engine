...
export function runCoreForecast(input: ForecastCaseInput): ForecastOutput {
  ...
  const posteriorProbability = normalizeProbability(
    signalPass.posteriorProbability
  );

  const posteriorOdds =
    probabilityToOdds(posteriorProbability) *
    env.priorMultiplier *
    env.posteriorMultiplier;
